    ...
    if not flag:
        ...
        if len(arr) == 1:
            ...
            if arr == 'jpg' or arr == 'png' or arr == 'jpeg':
                list1.append('Сжать изображения')
            elif arr == 'pdf':
                list1.append('PDF в Docx')
            elif arr == 'docx':
                list1.append('Docx в PDF')
        if len(arr) > 0:
...
def gms_window():
    ...
    layout = [
        ...
        [sg.Listbox(values=["Удалить по подстроке", "Удалить по началу названия", "Удалить по концу названия", "Удалить по расширению"],text_color='black', enable_events=True, size=(40, 10), key='-DELETE LIST-')],
...
def office():
    ...
    buttons1 = [[sg.Button('Выполнить', size=(8, 1), disabled=True, key="-ENTER-")],
                [sg.Button('Убрать', size=(8, 1), disabled=True, key='-CLEAR-')],
                [sg.Button('Убрать все', size=(8, 1), disabled=True, key='-CLEAR ALL-')]]
    buttons2 = [[sg.Button('Удалить по параметру', size=(8, 2), disabled=True, key='-DELETE-')],
